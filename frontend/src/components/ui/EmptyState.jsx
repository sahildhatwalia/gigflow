...
function EmptyState({
  ...
  const configs = {
    default: {
      ...
      color: "text-slate-800 bg-slate-50 dark:bg-slate-800/20 border-slate-200 dark:border-slate-700",
    },
    projects: {
      icon: FiLayers,
  ...
  return (
    <div className={`flex flex-col items-center justify-center text-center p-8 sm:p-12 bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-2xl ${className}`}>
      {/* Dynamic SVG Cues */}
      <div className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-6 border animate-pulse ${current.color}`}>
        <Icon size={28} />
      </div>
